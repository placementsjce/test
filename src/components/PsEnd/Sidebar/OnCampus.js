...
useEffect(() => {
  ...
  return <div>
      <div className='header_company'>ON CAMPUS</div>
            ...
            {oncampus?.map(function(oncamp) {return<><div className='eachupcomingcompany'><div className='upcomingcompanyname'>{oncamp.cname}</div>:<div>{oncamp?.date?.slice(0,10)?.split('-')?.reverse()?.join('-')}</div></div></>})}
